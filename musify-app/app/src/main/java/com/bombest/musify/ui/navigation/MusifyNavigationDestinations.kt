...
sealed class MusifyNavigationDestinations(val route: String) {
    ...
    object AlbumDetailScreen :
        ...
        fun buildRoute(albumSearchResult: SearchResult.AlbumSearchResult): String {
            ...
            val encodedId = URLEncoder.encode(albumSearchResult.id, StandardCharsets.UTF_8.toString())
            val encodedName = URLEncoder.encode(albumSearchResult.name, StandardCharsets.UTF_8.toString())
            val encodedArtists = URLEncoder.encode(albumSearchResult.artistsString, StandardCharsets.UTF_8.toString())
            val year = albumSearchResult.yearOfReleaseString.substringBefore("-").takeIf { it.isNotBlank() } ?: "Unknown"
            val encodedYear = URLEncoder.encode(year, StandardCharsets.UTF_8.toString())
    ...
    object LibraryScreen : MusifyNavigationDestinations("MusifyNavigationDestinations.LibraryScreen")
