  ...
  CustomPlayer({super.configuration})
    ...
    _subscriptions = [
      ...
      stream.error.listen((event) {
        ...
        final currentMedia = (currentIndex >= 0 &&
                currentIndex < state.playlist.medias.length)
            ? state.playlist.medias[currentIndex]
            : null;
        final currentUri = currentMedia?.uri ?? "unknown";
        AppLogger.reportError(
          '[MediaKitError] uri=$currentUri\n$event',
          StackTrace.current,
        );
      }),
    ];
