...
class SourcedTrack extends BasicSourcedTrack {
  ...
  static Future<SourcedTrack> fetchFromTrack({
    ...
    // Fallback: build a direct S3 source when no plugin is configured.
    if (audioSource == null || audioSourceConfig == null) {
      final fallbackStreams = _buildFallbackStreams(query);
      final item = SpotubeAudioSourceMatchObject(
        id: query.id,
        title: query.name,
        artists: query.artists.map((a) => a.name).toList(),
        duration: Duration(milliseconds: query.durationMs ?? 0),
        thumbnail: query.album?.images.firstOrNull?.url,
        externalUri: query.externalUri ?? '',
      );

      AppLogger.log.w(
        "[SourcedTrack] Using S3 fallback streams for ${query.id} url=${item.externalUri}",
      );
  ...
  String? get url {
    ...
    if (preferences.presets.isEmpty) {
      return sources.isNotEmpty ? sources.first.url : null;
    }
  ...
  SpotubeAudioSourceStreamObject? getStreamOfQuality(
    ...
    if (preset == null) return sources.first;
    ...
    if (matchingSources.isEmpty) {
      // No matching container; just use first available.
  ...
  String? getUrlOfQuality(
    ...
    final stream = getStreamOfQuality(preset, qualityIndex);
    final url = stream?.url ?? (sources.isNotEmpty ? sources.first.url : null);
    AppLogger.log.i(
      "[SourcedTrack] url=${url ?? 'null'} preset=${preset.name} qualityIndex=$qualityIndex sources=${sources.length}",
    );
    return url;
  }
  ...
  static List<SpotubeAudioSourceStreamObject> _buildFallbackStreams(
    SpotubeFullTrackObject query,
  ) {
    final url = query.externalUri ?? "";
    final ext = Uri.parse(url).path.split(".").last.toLowerCase();
    final isLossless = ext == "wav" || ext == "flac" || ext == "alac";
    final container = ext.isNotEmpty ? ext : "mp3";
    return [
      SpotubeAudioSourceStreamObject(
        url: url,
        container: container,
        type:
            isLossless ? SpotubeMediaCompressionType.lossless : SpotubeMediaCompressionType.lossy,
        codec: ext,
        // We don't know bitrate/sampleRate here; keep null to avoid misleading UI
      ),
    ];
  }
}
