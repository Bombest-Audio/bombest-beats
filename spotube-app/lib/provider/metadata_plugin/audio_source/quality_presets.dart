...
class AudioSourceAvailableQualityPresetsNotifier
  ...
  void _initialize(
    ...
    audioSourceConfigSnapshot.whenData((audioSourceConfig) {
      audioSourceSnapshot.whenData((audioSource) async {
        // Bombest S3 fallback: if no plugin configured, provide a basic preset so playback can proceed.
        if (audioSource == null || audioSourceConfig == null) {
          state = AudioSourcePresetsState(
            // Empty presets so player falls back to first source URL.
            presets: const [],
          );
          return;
        }
  ...
  void _updatePreferences() async {
    ...
    // If there's no plugin configured, persist under a generic key.
    final prefsKey = audioSourceConfig == null
        ? "audioSourceState-s3-fallback"
