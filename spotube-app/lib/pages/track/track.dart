...
class TrackPage extends HookConsumerWidget {
  ...
  Widget build(BuildContext context, ref) {
    ...
    // Check if this is an S3 track by checking if it exists in S3 tracks
    // This is more reliable than pattern matching
    final s3TracksQuery = ref.watch(s3TracksProvider);
    final isS3Track = s3TracksQuery.maybeWhen(
      data: (s3Tracks) => s3Tracks.any((t) => t.key == trackId),
      orElse: () => trackId.startsWith('music/') || 
                    trackId.contains('.wav') || 
                    trackId.contains('.mp3') ||
                    trackId.contains('.m4a') ||
                    trackId.contains('.flac'),
    );
    
    SpotubeFullTrackObject track;
    if (isS3Track) {
      // Fetch from S3 tracks provider
      track = s3TracksQuery.when(
        data: (s3Tracks) {
          final s3Track = s3Tracks.firstWhere(
            (t) => t.key == trackId,
            orElse: () => throw Exception('S3 track not found: $trackId'),
          );
          return s3TrackToSpotubeTrack(s3Track);
        },
        loading: () => FakeData.track, // Show skeleton while loading
        error: (_, __) => FakeData.track, // Fallback on error
      );
    } else {
      // Use metadata plugin provider for non-S3 tracks
      // But handle gracefully if no metadata plugin is set
      final trackQuery = ref.watch(metadataPluginTrackProvider(trackId));
      track = trackQuery.maybeWhen(
        data: (t) => t,
        orElse: () {
          // If metadata plugin is not available, try to find in S3 tracks as fallback
          return s3TracksQuery.maybeWhen(
            data: (s3Tracks) {
              try {
                final s3Track = s3Tracks.firstWhere(
                  (t) => t.key == trackId,
                ...
                return FakeData.track;
              }
            },
            orElse: () => FakeData.track,
          );
        },
      );
    }
          ...
          children: [
            ...
            Positioned.fill(
              child: BackdropFilter(
                ...
                child: Skeletonizer(
                  enabled: isS3Track 
                      ? ref.watch(s3TracksProvider).isLoading
                      : ref.watch(metadataPluginTrackProvider(trackId)).isLoading,
                  child: Container(
                    alignment: Alignment.topCenter,
                    decoration: BoxDecoration(
