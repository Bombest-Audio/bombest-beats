...
import 'package:spotube/services/health/health_check_service.dart';
import 'package:spotube/services/s3_sync/background_sync_service.dart';
...
// await FlutterDiscordRPC.initialize(Env.discordAppId);
    ...
    if (kIsDesktop) {
      await localNotifier.setup(appName: "Bombest Beats");
...
class Spotube extends HookConsumerWidget {
  ...
  Widget build(BuildContext context, ref) {
    ...
    // Bombest: disable plugin update prompts
    ...
    useEffect(() {
      // Remove native splash screen once Flutter is ready
      ...
      AppLogger.log.i('[Spotube.main] Performing health checks...');
      HealthCheckService.performHealthChecks().then(
        (result) {
          AppLogger.log.i('[Spotube.main] Health check: ${result.summary}');
          if (!result.isHealthy) {
            AppLogger.log.w('[Spotube.main] Health check issues detected: ${result.errors.join(", ")}');
          }
        },
        onError: (e) => AppLogger.log.w('[Spotube.main] Health check failed', error: e),
      );

      // Pre-warm S3 tracks provider on app startup
      AppLogger.log.i('[Spotube.main] Pre-warming s3TracksProvider...');
      ref.read(s3TracksProvider.future).then(
        (tracks) => AppLogger.log.i('[Spotube.main] Pre-warm completed: ${tracks.length} tracks'),
        onError: (e) => AppLogger.log.e('[Spotube.main] Pre-warm failed', error: e),
      );
